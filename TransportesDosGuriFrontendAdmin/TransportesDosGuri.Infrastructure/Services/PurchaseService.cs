...
namespace TransportesDosGuri.Infrastructure.Services
{
    public class PurchaseService : IPurchaseService
    {
        ...
        public async Task<List<PurchaseDTO>> GetAllAsync()
        {
            try
            {
                return await _http.GetFromJsonAsync<List<PurchaseDTO>>("/api/v1/Purchase") ?? new();
            }
        ...
        public async Task<PurchaseDTO?> GetByIdAsync(long id)
        {
            try
            {
                return await _http.GetFromJsonAsync<PurchaseDTO>($"/api/v1/Purchase/{id}");
            }
            catch (HttpRequestException ex) when (ex.StatusCode == HttpStatusCode.Unauthorized || ex.StatusCode == HttpStatusCode.Forbidden)
            {
                _navigationManager.NavigateTo("/error", replace: true);
        ...
        public async Task<(bool Success, string? Error)> DeleteAsync(long id)
        {
            ...
            try
            {
                ...
                return (false, error?.Message ?? "Não foi possível excluir a compra.");
            }
            catch
            {
                return (false, "Não foi possível excluir a compra.");
            }
        }
