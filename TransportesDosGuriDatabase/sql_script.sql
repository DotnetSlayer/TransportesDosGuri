...
ON [dbo].[Schedule] ([FlightId]);
GO

CREATE NONCLUSTERED INDEX [IX_Schedule_AirportId]
ON [dbo].[Schedule] ([AirportId]);
GO
...
- FK para AspNetUsers com ON DELETE CASCADE
  (apaga compras quando o usuário é excluído).
- Novos campos: AsaasPaymentId e AsaasInvoiceUrl.
===========================================================
*/
...
(
    ...
    [AsaasInvoiceUrl] NVARCHAR(500) NULL,
...
ON [dbo].[Purchase] ([ApplicationUserId]);
GO

CREATE NONCLUSTERED INDEX [IX_Purchase_TripId]
ON [dbo].[Purchase] ([TripId]);
GO

CREATE NONCLUSTERED INDEX [IX_Purchase_Status]
ON [dbo].[Purchase] ([Status]);
GO

CREATE NONCLUSTERED INDEX [IX_Purchase_PurchaseDate]
ON [dbo].[Purchase] ([PurchaseDate]);
GO

CREATE NONCLUSTERED INDEX [IX_Purchase_AsaasPaymentId]
ON [dbo].[Purchase] ([AsaasPaymentId]);
GO
...
(
    ...
    -- SEM cascade (a cascata chega via Purchase)
    CONSTRAINT [FK_Reservation_AspNetUsers]
        FOREIGN KEY ([ApplicationUserId])
        REFERENCES [dbo].[AspNetUsers] ([Id]),

    -- SEM cascade (assento é dado mestre)
    CONSTRAINT [FK_Reservation_FlightSeat]
        FOREIGN KEY ([FlightSeatId])
        REFERENCES [dbo].[FlightSeat] ([Id]),

    -- COM cascade (apaga reservas quando a Purchase é excluída)
...
ON [dbo].[UserRequest] ([ApplicationUserId]);
GO
...
PRINT 'Cascade delete habilitado em Purchase, Reservation (via Purchase) e UserRequest.';
GO